...
class Bird {
  ...
  shoot() {
    if (shootTheBird) {
      ...
      this.position.y =
        canvas.height - (Vy * t - 0.5 * g * t * t) - slingShotHeight;
      this.draw();
    }
  }
}
      ...
      for (let i = birds.length - 1; i >= 0; i--) {
        ...
        for (let j = tnts.length - 1; j >= 0; j--) {
          ...
          if (
            ...
            // Plaing explosion Sound
            playSound(tntexplosion);

            setTimeout(() => {
              // Hide the GIF after 1 sec
...
function handleMouseMove(event) {
  if (birdToShoot != null) {
    ...
    if (distance < 130) {
      ...
      birdToShoot.position.y =
        ground - slingShotHeight - m * ratio * maxPower - 30;
    }
  }
}
